#!/bin/bash
# simple test for 'subvol show' output, copy of 010 with JSON output
...
# no limits
run_check $SUDO_HELPER "$TOP"/btrfs --format json subvolume show "$TEST_MNT"
# 1G limit for exclusive
run_check $SUDO_HELPER "$TOP"/btrfs --format json subvolume show "$TEST_MNT"/subv1
# no limits
run_check $SUDO_HELPER "$TOP"/btrfs --format json subvolume show "$TEST_MNT"/subv2
